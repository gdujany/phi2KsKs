...
# from LHCbStyle import printLHCb
...
for track in ["DD"]:#["DD","LL","DD-DTF", "LL-DTF","DeltaTauLL","DeltaTauLD", "DeltaTauLL-DTF"]:
	...
	canvas.cd()
	if track == "LL-DTF":
		t.Draw("(phi_DTF_CTAU_Ks1_PV/299.792458-Ks1_TRUETAU)*1000. >>histo(50,0,5)","phi_DTF_CTAU_Ks1_PV >= 0 && Ds_DTF_CHI2_PV >0 && phi_BKGCAT==0 && pi1_TRACK_Type ==3 && pi2_TRACK_Type ==3")
		histo=r.gROOT.FindObject("histo")
		histo.SetDirectory(0)
		t.Draw("(phi_DTF_CTAU_Ks2_PV/299.792458-Ks2_TRUETAU)*1000. >>histo2(50,0,5)","phi_DTF_CTAU_Ks2_PV >= 0 && Ds_DTF_CHI2_PV >0 && phi_BKGCAT==0 && pi3_TRACK_Type ==3 && pi3_TRACK_Type ==3")
		histo2=r.gROOT.FindObject("histo2")
		histo2.SetDirectory(0)
		histo.Add(histo2)
	elif track == "DD-DTF":
		t.Draw("(phi_DTF_CTAU_Ks1_PV/299.792458-Ks1_TRUETAU)*1000. >>histo(100,0,10)","phi_DTF_CTAU_Ks1_PV >= 0 && Ds_DTF_CHI2_PV >0 && phi_BKGCAT==0 && pi1_TRACK_Type ==5 && pi2_TRACK_Type ==5")
		histo=r.gROOT.FindObject("histo")
		histo.SetDirectory(0)
		t.Draw("(phi_DTF_CTAU_Ks2_PV/299.792458-Ks2_TRUETAU)*1000. >>histo2(100,0,10)","phi_DTF_CTAU_Ks2_PV >= 0 && Ds_DTF_CHI2_PV >0 && phi_BKGCAT==0 && pi3_TRACK_Type ==5 && pi3_TRACK_Type ==5")
		histo2=r.gROOT.FindObject("histo2")
		histo2.SetDirectory(0)
		histo.Add(histo2)
	elif track == "LL":
		...
		histo=r.gROOT.FindObject("histo")
		histo.SetDirectory(0)
		t.Draw("(Ks2_TAU-Ks2_TRUETAU)*1000. >>histo2(6,-3,3)","Ks2_TAU >= 0 && phi_BKGCAT==0 && pi3_TRACK_Type ==3 && pi3_TRACK_Type ==3")
		histo2=r.gROOT.FindObject("histo2")
		histo2.SetDirectory(0)
		histo.Add(histo2)
	elif track == "DD":
		...
		t.Draw("(phi_DTF_CTAU_Ks1_PV/299.792458-phi_DTF_CTAU_Ks2_PV/299.792458)*1000.-(Ks1_TRUETAU-Ks2_TRUETAU)*1000.>>histo(40,-10,10)","Ks1_TAU >= 0 && Ks2_TAU >=0 && phi_BKGCAT==0 && ((pi1_TRACK_Type ==5 && pi2_TRACK_Type ==5  && pi3_TRACK_Type ==3 && pi4_TRACK_Type ==3)||(pi1_TRACK_Type ==3 && pi2_TRACK_Type ==3  && pi3_TRACK_Type ==5 && pi4_TRACK_Type ==5))")
		histo=r.gROOT.FindObject("histo")
		histo.SetDirectory(0)
	histo.SetName("Time resolution - "+track)
	histo.SetTitle("D_{s}^{#pm} #rightarrow #phi #pi^{#pm}")
             
	if track == "LL-DTF":
		f = r.TF1("f","[0]/(x^2 + [1]^2/4)  ",0,1.5)
		...
		f.SetParameter(0, 3.28120e-04)
		f.SetParameter(1,1)
		f.SetParLimits(1,0,1.5)
		f.SetParameter(2,1.43076e-03)
		f.SetParName(0,"a")
		f.SetParName(1,"#Gamma")
		f.SetParName(2,"#mu")
	elif track == "DeltaTauLL-DTF":
		f = r.TF1("f","[0]/((x-[2])^2 + [1]^2/4) ",-10,10);
		f.SetParameter(0, 3.28120e-04)
		f.SetParameter(1,1.43076)
		f.SetParLimits(1,0,4)
		f.SetParameter(2,0)
		f.SetParName(0,"a")
		f.SetParName(1,"#Gamma")
		f.SetParName(2,"#mu")
	...
	canvas.Update()
	printLHCb(x=0.35)
	canvas.Print("time_res_Ds/timeResolution-"+track+".pdf")
...
for track in ["LL","DD","all","DD-DTF", "LL-DTF","all-DTF"]:
	...
	h_a.GetXaxis().SetTitle("t(true) [ps]") 
  	...
  	h_a.SetTitle("D_{s}^{#pm} #rightarrow #phi #pi^{#pm}")
  	canvas = r.TCanvas("canvas","Canvas Title")
  	h_a.Draw("p");
	canvas.cd()
	ntrack1 = "{"+track+"1}"
	...
	if "DTF" in track:
		t.Draw("(phi_DTF_CTAU_Ks1_PV/299.792458-Ks1_TRUETAU)*1000:Ks1_TRUETAU*1000","phi_DTF_CTAU_Ks1_PV >= 0 && Ds_DTF_CHI2_PV >0 && phi_BKGCAT==0"+nntrack1, "same")
		t.Draw("(phi_DTF_CTAU_Ks2_PV/299.792458-Ks2_TRUETAU)*1000:Ks2_TRUETAU*1000","phi_DTF_CTAU_Ks2_PV >= 0 && Ds_DTF_CHI2_PV >0 && phi_BKGCAT==0"+nntrack2, "same")
	else:
	...
	canvas.Update()
	printLHCb()
	canvas.Print("time_res_Ds/"+track+".pdf")
	del canvas
	del h_a

	if "DTF" in track:
		t.Draw("(phi_DTF_CTAU_Ks1_PV/299.792458-Ks1_TRUETAU)*1000:Ks1_TRUETAU*1000 >>histo(20,0,"+maximum+")","phi_DTF_CTAU_Ks1_PV >= 0 && Ds_DTF_CHI2_PV >0 && phi_BKGCAT==0"+nntrack1, "P0 prof")
		histo=r.gROOT.FindObject("histo")
		histo.SetDirectory(0)
		t.Draw("(phi_DTF_CTAU_Ks2_PV/299.792458-Ks2_TRUETAU)*1000:Ks2_TRUETAU*1000 >>histo2(20,0,"+maximum+")","phi_DTF_CTAU_Ks2_PV >= 0 && Ds_DTF_CHI2_PV >0 && phi_BKGCAT==0"+nntrack2, "P0 prof")
		histo2=r.gROOT.FindObject("histo2")
		histo2.SetDirectory(0)
		histo.Add(histo2)
		histo.SetTitle("D_{s}^{#pm} #rightarrow #phi #pi^{#pm}")
		...
		canvas.Update()
		canvas.Print("time_res_Ds/"+track+"-prof.pdf")
	else:
