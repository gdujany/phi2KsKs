...
from StandardParticles import StdAllLooseKaons, StdAllLooseMuons, StdAllLoosePions
...
def VMaker(_name,_RequiredSelections,_DecayDescriptor,_Filter,conf,_prescale):
    ...
    if 'D_s+' in _DecayDescriptor and 'phi(1020)' in _DecayDescriptor:
        #_CombiCut += " (APT > %(Dp_PT_MIN)s *MeV) & (ADAMASS('D_s+') < %(Dp_MASS_WIN)s + 30*MeV) & (ACUTDOCACHI2(%(Dp_DOCACHI2_MAX)s,''))" %conf
        #_MotherCut = "(DMASS('D_s+') < %(Dp_MASS_WIN)s + 20*MeV) & (VFASPF(VCHI2/VDOF) < %(Dp_VCHI2NDOF_MAX)s) & (MIPCHI2DV(PRIMARY) < %(Dp_IPCHI2_MAX)s)" %conf
        #_MassFilter = FilterDesktop(name = "MassFilter_"+_name,Code = "(DMASS('D_s+') < %(Dp_MASS_WIN)s *MeV)" %conf)
        #_MassFilterDTF = FilterDesktop(name = "MassFilterDTF_"+_name,Code = "(abs(DTF_FUN(M,True,'D_s+') - 1970) < %(Dp_MASS_WIN)s *MeV)" %conf)
        _CombiCut += " (APT > %(Dp_PT_MIN)s *MeV) & (ADAMASS(1920*MeV) < %(Dp_MASS_WIN)s + 30*MeV) & (ACUTDOCACHI2(%(Dp_DOCACHI2_MAX)s,''))" %conf
        _MotherCut = "(DMASS(1920*MeV) < %(Dp_MASS_WIN)s + 20*MeV) & (VFASPF(VCHI2/VDOF) < %(Dp_VCHI2NDOF_MAX)s) & (MIPCHI2DV(PRIMARY) < %(Dp_IPCHI2_MAX)s)" %conf
        _MassFilter = FilterDesktop(name = "MassFilter_"+_name,Code = "(DMASS(1920*MeV) < %(Dp_MASS_WIN)s *MeV)" %conf)
        #_MassFilterDTF = FilterDesktop(name = "MassFilterDTF_"+_name,Code = "(abs(DTF_FUN(M,True,'D_s+') - 1970) < %(Dp_MASS_WIN)s *MeV)" %conf)
        _MassFilterDTF = FilterDesktop(name = "MassFilterDTF_"+_name,Code = "(DMASS(1920*MeV) < %(Dp_MASS_WIN)s *MeV)" %conf)
    elif 'phi(1020)' in _DecayDescriptor:
